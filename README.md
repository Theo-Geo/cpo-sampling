...
The second order structure tensor with respect to any orientation distribution function (ODF) $n(\theta,\phi)$ of normal axes $\boldsymbol{n}$ is defined as
```math
\boldsymbol{a}^{(2)}_n = \int_{S^2}\boldsymbol{n}\otimes\boldsymbol{n}\ n(\theta, \phi) \ \mathrm{d}\Omega
```
and can be approximated by a discrete ensemble of axes $\{\boldsymbol{n}_i\}_{i=1}^{N_g}$ with volume fractions $f_i$ that sum up to one,
```math
\boldsymbol{a}^{(2)}_n = \sum_i^{N_g}\boldsymbol{n}_i\otimes\boldsymbol{n}_i f_i
```
...
```math
\begin{align}
    \Delta_i(\phi_1, \theta, \phi_2) &:= \boldsymbol{a}^{(2)}_{\rm obs, i} - \boldsymbol{a}^{(2)}_{\rm modeled, i}(\phi_1, \theta, \phi_2)\qquad \qquad i = (100,\ 010,\ 001) \\
...
\end{align}
```
with $\sigma_\Delta$ being the allowed standart deviation of $\Delta$, which in turn means the allowed standart error of $\boldsymbol{a}^{(2)}_{\rm modeled, i}$ with respect to $\boldsymbol{a}^{(2)}_{\rm obs, i}$. The standart error on $\boldsymbol{a}^{(2)}_{\rm obs}$ is an error on the mean and scales with the number of samples (or in this case grains) as $\sigma_{a^{(2)}} = \sigma_{\boldsymbol{n}_i\otimes \boldsymbol{n}_i}/\sqrt{N_g}$. The error on each component of the structure tensor is assumed to be the same and can be prescribed with a tolerance value. If the fluctuations in each component of $\Delta$ are on the order of $\Delta \sim \sigma_{\Delta}$ the misfit is one the order of one.
